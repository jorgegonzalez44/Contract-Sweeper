"""Tests for scripts/download_ed.py."""
import logging
import sys
...
logging.getLogger("download_ed").setLevel(logging.CRITICAL)
...
class TestResultsToDf:
    ...
    def test_source_dataset_is_ed(self):
        records = [{"Award ID": "ED-001"}]
        df = _results_to_df(records, "test.csv")
        assert df["source_dataset"].iloc[0] == "ed"

    def test_master_columns_complete(self):
        records = [{"Award ID": "ED-002"}]
        df = _results_to_df(records, "test.csv")
...
class TestRunCaching:
    def test_run_returns_dict(self, tmp_path):
        ...
        (tmp_path / "data" / "staging" / "raw" / "ed").mkdir(parents=True)
        (tmp_path / "data" / "staging" / "processed").mkdir(parents=True)
        with patch("scripts.download_ed._paginate", return_value=[]), \
             patch("scripts.download_ed._session", return_value=MagicMock()):
            from scripts.download_ed import _run
            result = _run(root=tmp_path, force=False, fy_start=2025)
        assert "master_rows" in result

    def test_file_caching_skips_fetch(self, tmp_path):
        raw_dir = tmp_path / "data" / "staging" / "raw" / "ed"
        raw_dir.mkdir(parents=True)
        from scripts.download_ed import TIME_WINDOWS
        for window in TIME_WINDOWS[:1]:
            for ft in ("pop", "recipient"):
                f = raw_dir / f"ed_{ft}_{window['label']}.csv"
                f.write_text("col\nrow\n")
        assert _file_has_data(raw_dir / f"ed_pop_{TIME_WINDOWS[0]['label']}.csv") is True
