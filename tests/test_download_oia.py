"""Tests for scripts/download_oia.py."""
import logging
import sys
...
logging.getLogger("download_oia").setLevel(logging.CRITICAL)

from scripts.download_oia import MASTER_COLUMNS, _derive_fiscal_year, _file_has_data
...
class TestRunCaching:
    def test_run_with_mocked_paginate(self, tmp_path):
        (tmp_path / "data" / "staging" / "raw" / "oia").mkdir(parents=True)
        (tmp_path / "data" / "staging" / "processed").mkdir(parents=True)
        with patch("scripts.download_oia._paginate", return_value=[]), \
             patch("scripts.download_oia._session", return_value=MagicMock()):
            from scripts.download_oia import _run
