"""Tests for scripts/download_hhs.py."""
import logging
import sys
...
logging.getLogger("download_hhs").setLevel(logging.CRITICAL)

from scripts.download_hhs import MASTER_COLUMNS, _derive_fiscal_year, _file_has_data
...
class TestRunCaching:
    def test_run_with_mocked_paginate(self, tmp_path):
        (tmp_path / "data" / "staging" / "raw" / "hhs").mkdir(parents=True)
        (tmp_path / "data" / "staging" / "processed").mkdir(parents=True)
        with patch("scripts.download_hhs._paginate", return_value=[]), \
             patch("scripts.download_hhs._session", return_value=MagicMock()):
            from scripts.download_hhs import _run
